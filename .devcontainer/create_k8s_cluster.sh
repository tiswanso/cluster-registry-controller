...
CLUSNM=${CLUSNM:-example}
K8SAPIPORT=${K8SAPIPORT:-9000}
...
kind_running="$(docker inspect -f '{{.State.Running}}' "${CLUSNM}-control-plane" 2>/dev/null || true)"
if [ "${kind_running}" != 'true' ]; then
cat <<EOF | kind create cluster --name ${CLUSNM} --config=-
kind: Cluster
